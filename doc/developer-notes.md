...
- [Development guidelines](#development-guidelines)
    - [General LeisureCoin Core](#general-LeisureCoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Run with the `-testnet` option to run with "play LeisureCoins (tLeisureCoin)" on the test network, if you
...
LeisureCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/LeisureCoin-official/LeisureCoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_LeisureCoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_LeisureCoin --log_level=test_suite
$ valgrind -v --leak-check=full src/LeisureCoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_LeisureCoin.coverage/index.html`.
```

**Sanitizers**

LeisureCoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as LeisureCoin Core, where everyone uses
...
pay attention to for reviewers of LeisureCoin Core code.

General LeisureCoin Core
--------------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: LeisureCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of LeisureCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against LeisureCoin Core so that they can be integrated
...
file descriptors it will cause problems with LeisureCoin's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to LeisureCoin's
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:LeisureCoin/LeisureCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `LeisureCoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `LeisureCoin-cli` and the GUI debug console use this table to determine how to
